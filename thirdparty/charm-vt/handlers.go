...
func (e *Emulator) registerDefaultCcHandlers() {
	for i := byte(ansi.NUL); i <= ansi.US; i++ {
		...
		case ansi.SO:
			e.registerCcHandler(i, func() bool { e.gl = 1; return true })
		case ansi.SI:
			e.registerCcHandler(i, func() bool { e.gl = 0; return true })
		}
	}
...
func (e *Emulator) registerDefaultEscHandlers() {
	...
	e.RegisterEscHandler('N', func() bool { e.gsingle = 2; return true })
	e.RegisterEscHandler('O', func() bool { e.gsingle = 3; return true })
...
func (e *Emulator) registerDefaultCsiHandlers() {
	e.RegisterCsiHandler(ansi.Command(0, '!', 'p'), func(_ ansi.Params) bool {
		e.softReset()
		return true
	})
	...
	e.RegisterCsiHandler('J', func(params ansi.Params) bool {
		...
		case 0: // Erase screen below (from after cursor position)
			...
			e.scr.eraseArea(rect1)
			e.scr.eraseArea(rect2)
			e.scr.clearWraps(rect1)
			e.scr.clearWraps(rect2)
		case 1: // Erase screen above (including cursor)
			e.scr.eraseArea(uv.Rect(0, 0, width, y))
			e.scr.eraseArea(uv.Rect(0, y, x+1, 1))
			e.scr.clearWraps(uv.Rect(0, 0, width, y+1))
			if x+1 >= width {
				e.scr.setWrapped(y+1, false)
			}
		case 2: // erase screen
			// ED erases the viewport, not a scroll operation. Match the
			// browser's in-place clear and retain the current erase background.
			e.scr.FillArea(e.scr.blankCell(), e.scr.Bounds())
		case 3: // Erase saved lines without changing the visible screen.
	...
	e.RegisterCsiHandler('K', func(params ansi.Params) bool {
		...
		case 0: // Erase from cursor to end of line
			...
			if x == 0 {
				e.scr.setWrapped(y, false)
			}
		case 1: // Erase from start of line to cursor
			rect := uv.Rect(0, y, x+1, 1)
			e.scr.eraseArea(rect)
	...
	e.RegisterCsiHandler('f', func(params ansi.Params) bool {
		...
		row, _, _ := params.Param(0, 1)
		col, _, _ := params.Param(1, 1)
		y := min(height-1, max(1, row)-1)
		x := min(width-1, max(1, col)-1)
		e.setCursorPosition(x, y)
		return true
	})
	...
	e.RegisterCsiHandler('u', func(_ ansi.Params) bool {
		// Restore Current Cursor Position [ansi.SCORC]. Prefix-bearing
		// keyboard protocol sequences have distinct handler keys.
		e.restoreCursor()
		return true
	})

...
func TestScrollback(t *testing.T) {
	...
	t.Run("ED 2 does not manufacture scrollback", func(t *testing.T) {
		e := NewEmulator(20, 5)
		...
		// Erasing in place must not add history absent from the browser.
		newLen := e.ScrollbackLen()
		if newLen != initialLen {
			t.Errorf("ED 2 changed scrollback, was %d now %d", initialLen, newLen)
		}

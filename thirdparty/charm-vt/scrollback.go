...
func (s *Scrollback) Push(line uv.Line) {
	s.pushWrapped(line, false, true)
}

func (s *Scrollback) pushWrapped(line uv.Line, wrapped, known bool) {
	if s == nil || s.maxLines <= 0 {
		return
	}
	if !known || (len(s.lines) > 0 && len(s.wrapped) != len(s.lines)) {
		s.wrapped = nil
	} else {
		if len(s.wrapped) >= s.maxLines {

...
type Emulator struct {
	...
	// The preceding glyph ends at the right margin, even with autowrap off.
	// The next spacing character wraps only when autowrap is enabled.
	atPhantom bool
}
...
func NewEmulator(w, h int) *Emulator {
	...
	// One sentinel byte distinguishes an accepted full payload from overflow.
	// The upstream parser otherwise silently truncates at the buffer capacity.
	t.parser.SetDataSize(maxControlStringBytes + 1)
...
// DisableScrollback releases and disables history on both screens. Visible
// grids, cursor state, and parser state are unchanged.
func (e *Emulator) DisableScrollback() {
	e.scrs[0].SetScrollback(nil)
	e.scrs[1].SetScrollback(nil)
}

...
export const RemoteDesktopToolbar: React.FC<ToolbarProps> = ({
  ...
  if (toolbar.hidden) {
    return (
      <button
        type="button"
        className="absolute top-0 z-30 h-7 px-2 border border-t-0 border-ide-border bg-ide-panel text-xs text-ide-text"
    ...
    >
      {!toolbar.collapsed && (
        <div className="mt-2 flex max-w-[calc(100vw-32px)] items-center overflow-x-auto border border-ide-border bg-ide-panel shadow-sm">
          <button type="button" className={buttonClass} onClick={() => onToolbarChange({ pinned: !toolbar.pinned })}>
      ...
      <button
        type="button"
        className="h-6 min-w-20 border border-t-0 border-ide-border bg-ide-panel text-ide-mute hover:text-ide-text"
        onPointerDown={(event) => {
...
const ToolbarMenuButton: React.FC<{ active: boolean; icon: React.ReactNode; label: string; onClick: () => void }> = ({
  ...
  <button
    type="button"
    className={`h-8 px-2 flex items-center gap-1.5 border-y border-r border-ide-border text-xs ${
...
const ClipboardMenu: React.FC<{
  ...
  <div className="grid gap-2">
    <textarea
      className="h-24 resize-none border border-ide-border bg-ide-bg p-2 text-xs outline-none"
...
const MonitorLayout: React.FC<{
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="relative h-20 w-56 border border-ide-border bg-ide-bg">
    ...
    <div className="grid" style={{ gridTemplateColumns: `repeat(${options.length}, minmax(0, 1fr))` }}>
      {options.map(([key, text]) => (
        <button
          key={key}
          type="button"
          className={`h-8 border border-ide-border text-xs ${value === key ? "bg-ide-accent text-ide-on-accent" : "bg-ide-bg hover:bg-ide-border/50"}`}
...
const SliderRow: React.FC<{
  ...
  <label className="grid grid-cols-[72px_1fr_46px] items-center gap-2">
    <span className="text-ide-mute">{label}</span>
    <Slider value={[value]} min={min} max={max} step={1} onValueChange={([next]) => onValue(next ?? value)} />
...
const ToggleButton: React.FC<{
  ...
  <button
    type="button"
    className={`h-8 px-2 flex items-center justify-center gap-1.5 border border-ide-border text-xs disabled:opacity-40 ${
